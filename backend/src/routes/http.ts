...
export function badRequest(reply: FastifyReply, message: string, code?: string): FastifyReply {
  return reply.code(400).send(code ? { error: message, code } : { error: message });
}

export function conflict(reply: FastifyReply, message: string, code?: string): FastifyReply {
